...
Полный пересчёт на ноутбуке занимает около 5 секунд одной командой. Технические подробности
...
- **Экран просмотра** — один HTML-файл, работает без интернета.
...
uv run moneygraph        # полный пересчёт: data/*.parquet → out/  (~5 секунд)
```
...
| Файл | Что внутри |
|---|---|
| **`nodes_roles.csv`** | все 2 248 узлов: роль, уверенность, кластер, приоритет, обоснование с числами, а также сработавшее правило и метрики |
...
| **Распределитель** | ≥10 получателей, и получателей минимум втрое больше, чем плательщиков | веерная раздача: выплаты участникам или обналичивание |
| **Консолидатор** | ≥4 разных плательщика **или** ≥2 seed среди плательщиков | точка сбора денег от нескольких участников; отвечает на вопрос ТЗ «кто собирает деньги нескольких seed» |
| **Транзит** | деньги приходят и уходят дальше: отправлено не меньше половины полученного или ≥80% ушло в течение 2 дней | счёт, через который деньги прогоняют |
...
**Уверенность (`role_score`)** равна 0,5, когда узел ровно на пороге, и растёт до 1,0 при
...
| Сигналы | 10% | быстрый транзит, всплески, циклы, повторяющиеся маршруты и суммы, аномалии |
...
> 12,44 млн ₸); признаки консолидации: …8165763100 собирает от 15 плательщиков (1,17 млн ₸),
> в кластере 3 seed.
...
**Экран** (`out/viewer.html`):

- поиск по gid: полный номер или любые его цифры. Все gid заканчиваются на `…100`, поэтому удобнее
  вводить 8–10 последних цифр;
- схема окрестности выбранного узла: слева кто платит, справа кому уходит, стрелки показывают
  направление, цвет — роль, seed отмечены ромбом, узлы 4-го колена — пунктиром;
- режим «весь граф» и фильтры по ролям и кластерам;
- карточка узла: роль, правило, обоснование, разложение приоритета, кластер, контрагенты с суммами
  и датами. Любого контрагента можно открыть кликом;
- кликабельные списки «Топ-50» и «Кластеры».

**AI-ассистент** (при запуске через `uv run moneygraph serve`) отвечает на вопросы на обычном языке,
например «кто собирает деньги от нескольких seed?» или «почему этот узел координатор и кого
проверить рядом?». Ассистент сам обращается к результатам расчёта, отвечает со ссылками на
конкретные gid (они кликабельны на схеме) и формулирует выводы как гипотезы. Используется
OpenAI; подключить другую модель, например Claude, можно без переделки остального.
...
| Временные паттерны | сквозной транзит (пришло и ушло за 1–2 дня), синхронные поступления от нескольких плательщиков в один день, веерная рассылка за день |
...
1. `uv run moneygraph`: живой пересчёт за ~5 секунд, проверка схемы ТЗ проходит.
