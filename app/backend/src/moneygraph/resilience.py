...
def resilience(G: nx.DiGraph, f: pd.DataFrame) -> tuple[dict, pd.DataFrame]:
    ...
    base = _stats(G, seeds)
    base_info = {"reach": base[0], "flow": round(base[1]), "lcc": base[2], "components": base[3]}
    ...
    rows = []
    for n in [n for n in C.RESILIENCE_N if n <= min(len(order), len(pool))]:
        H = G.copy()
        ...
        rows.append(
            dict(
                removed_top_n=n,
                reach_top=round(top[0] / base[0], 4),
                flow_top=round(top[1] / base[1], 4),
                lcc_top=top[2],
                components_top=top[3],
                reach_random=round(rnd[0] / base[0], 4),
                flow_random=round(rnd[1] / base[1], 4),
                lcc_random=round(rnd[2]),
                components_random=round(rnd[3], 1),
            )
        )
    return base_info, pd.DataFrame(rows)
