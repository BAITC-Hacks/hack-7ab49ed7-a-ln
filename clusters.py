"""
Кластеры: Louvain на НЕНАПРАВЛЕННОЙ проекции (встречные суммы складываются), seed=42.
Направление теряется только здесь; роли и приоритет считаются на направленном графе.
Кластер 0 — узлы без единого перевода в выгрузке.
"""
...
def louvain(G: nx.DiGraph) -> dict:
    ...
    comms = nx.community.louvain_communities(UG.subgraph(connected), weight="weight",
                                             seed=C.RANDOM_SEED, resolution=C.LOUVAIN_RESOLUTION)
...
def cluster_table(G: nx.DiGraph, f: pd.DataFrame) -> pd.DataFrame:
    internal = {}
    for u, v, d in G.edges(data=True):
    ...
    for c, grp in f.groupby("cluster_id"):
        ...
        rows.append(dict(
            cluster_id=int(c), n_nodes=len(grp), n_seed=int(grp.is_seed.sum()),
            sum_kzt_internal=round(internal.get(c, 0.0), 2),
            top_gids=";".join(str(g) for g in top.index[:5]),
            hypothesis=hypothesis(c, grp, top, internal.get(c, 0.0)),
...
def hypothesis(c, grp, top, internal) -> str:
    if c == 0:
        return (f"{plural(len(grp), 'узел', 'узла', 'узлов')} без переводов ≥5 000 ₸ внутри банка за июль "
    ...
    if n_seed >= 2 and len(hubs):
        h = hubs.iloc[0]
        txt = (f"Контур сбора: {n_seed} seed; средства сходятся к узлу {hubs.index[0]} "
        ...
        txt = (f"Контур распределения: веер от {fans.index[0]} на {int(fans.out_deg.iloc[0])} получателей "
               f"({kzt(fans.out_kzt.iloc[0])}) — возможны выплаты/обналичивание; seed в кластере: {n_seed}")
    elif len(hubs):
        h = hubs.iloc[0]
        txt = (f"Локальный узел сбора {hubs.index[0]} ({ROLE_HYPOTHESIS[h.role]}, "
        ...
        txt = (f"Окружение без seed ({len(grp)} узлов, колено {int(grp.depth.min())}–{int(grp.depth.max())}): "
               f"вероятно, бытовые/коммерческие получатели; ключевой — {lead_txt}")
    trunc = grp.truncated.mean()
    if trunc >= 0.3:
        txt += f"; {pct(trunc)} узлов — обрыв 4-го колена, нужна дозагрузка исходящих"
    return txt
