"""Форматирование чисел и русские подписи. Все тексты — гипотезы, а не утверждения."""
import math
...
FLAG_RU = {
    ...
    "isolated": "Нет переводов ≥5 000 ₸ в выгрузке",
    "truncated": "Обрыв обхода на 4-м колене: исходящие не выгружались",
...
def ratio_txt(r: float) -> str:
    if r is None or (isinstance(r, float) and math.isnan(r)):
        return "—"
    return pct(r) if r <= 9.99 else f"×{r:.0f}"


def fit_text(parts, limit: int = 200, sep: str = "; ") -> str:
    """Склеивает фразы, пока влезает в limit символов (первая фраза обязательна)."""
    out = parts[0]
    for p in parts[1:]:
